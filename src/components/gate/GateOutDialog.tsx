...
import type { Vehicle, Container } from "@/types";
...
interface GateOutDialogProps {
  ...
  loading: boolean;
  vehicle?: (Vehicle & { currentContainer?: string }) | null; // Add optional vehicle prop
    ...
    try {
      ...
      if (isContainerRequired && !cNum) {
        form.setError("containerNumber", {
          message: "Container number is required for this operation.",
        });
        setIsVerifying(false);
        return;
      }
      ...
      if (cNum) {
        ...
        const currentContainer = (
          Array.isArray(containers) ? containers[0] : null
        ) as Container | null;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Process vehicle exit from terminal
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
